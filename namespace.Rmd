...
If you are using functions repeatedly, you can avoid `::` by importing the function by adding `@importFrom pkg fun` to the `NAMESPACE`. This also has a small performance benefit, because `::` adds approximately 5 µs to function evaluation time. Operators can also be imported in a similar manner, e.g., `@importFrom magrittr %>%`.
